...
const handleLogin = async () => {
  try {
    ...
    // 调用后端登录接口
    const response = await fetch('http://localhost:8083/catcatecutomer/users/login', {
      method: 'POST',
      headers: {
    ...
    // 检查登录是否成功（后端返回完整的用户对象）
    if (result && result.id && result.token) {
      ElMessage.success('登录成功，正在进入系统...');

      // 存储用户信息和token
      localStorage.setItem('catCafeUserInfo', JSON.stringify({
        id: result.id,
...
const handleRegister = async () => {
  try {
    ...
    // 实际项目中替换为API调用
    const response = await fetch('http://localhost:8083/catcatecutomer/users/register', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
